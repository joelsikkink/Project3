...
public class Cells {
		...
		for (int board = 0; board < boards; board ++) {
			for (int row = 0; row < 9; row++) {
				for (int column = 0; column < 9; column++) {
					...
					cellValue[row][column][board] = EZ.addText(cells[row][column][board].getXCenter(), cells[row][column][board].getYCenter(),
							"", Color.black, 40);
				}
			}
		}
	...
	private void initializeSam() {
		for (int board = 0; board < boards; board++) {
			...
			int posY = positionY;
			switch(board) {
			case(1): posX += 480; break;
			case(2): posY += 480; break;
			case(3): posX += 480; posY += 480; break;
			case(4): posX += 240; posY += 240; break;
			default: break;
			}
			for (int row = 0; row < 9; row++) {
				for (int column = 0; column < 9; column++) {
					...
					if (board == 4) {
						if ((column >= 3 && column < 6) || (row >=3 && row < 6)) {
							...
							cells[row][column][board] = EZ.addRectangle(-50, -50, 38, 38, Color.WHITE, true); // position unwanted cells off screen to prevent null pointer exception errors
							cellValue[row][column][board] = EZ.addText(cells[row][column][board].getXCenter(), cells[row][column][board].getYCenter(),
									"", Color.black, 35);
						}
					} else {
